...
class StarGAN_v2():
    ...
    def refer_canvas(self, x_real, x_ref, y_trg, path, img_num):
        if type(img_num) == list:
            # In test phase
            src_img_num = 1 # min(img_num[0], self.batch_size)
            ref_img_num = min(img_num[1], self.batch_size)
    ...
    def test(self, merge=True, merge_size=0):
        ...
        for idx, domain in enumerate(self.domain_list):
            if domain == 'photo':
                ...
                # print(image_list)
                # reference_images.append(image_list[2])
                # reference_domain.append(domain_list[2])
        
        # print("The reference image size is: {}".format(len(reference_images)))
        ...
        if merge:
            ...
            for src_img_path in tqdm(source_images):
                ...
                # for ref_img_path, ref_img_domain in zip(reference_images, reference_domain):
                #     # ref_img_path = reference_images[0]
                #     # ref_img_domain = reference_domain[0]
                ...
                #     x_real = src_img[:src_img_num]
                #     x_ref = ref_img # [:ref_img_num]
                #     y_trg = ref_img_domain # [:ref_img_num]

                #     canvas = PIL.Image.new('RGB', (self.img_size, self.img_size), 'white')

                #     x_real_post = postprocess_images(x_real)
                #     x_ref_post = postprocess_images(x_ref)

                #     dst_image = x_ref_post

                #     row_images = np.stack([dst_image] * src_img_num)
                #     row_images = preprocess_fit_train_image(row_images)
                #     row_images_y = np.stack([y_trg[0]] * src_img_num)

                #     s_trg = self.style_encoder_ema([row_images, row_images_y])
                #     row_fake_images = postprocess_images(self.generator_ema([x_real, s_trg]))

                #     image = row_fake_images
                #     canvas.paste(PIL.Image.fromarray(np.uint8(image[0]), 'RGB'),
                #                         (0, 0))

                #     if not os.path.exists('{}/{}'.format(self.result_dir, ref_img_domain)):
                #         os.makedirs('{}/{}'.format(self.result_dir, ref_img_domain))

                #     save_path = '{}/{}/{}{}'.format(self.result_dir, ref_img_domain, pic_num, src_extension)
